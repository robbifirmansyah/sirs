...
type MasterPoli = { id: string, kode_poli: string, nama_poli: string }
type LaporanDetail = { poli_id: string, dalam_kota_laki: string, dalam_kota_perempuan: string, luar_kota_laki: string, luar_kota_perempuan: string, hari_buka: string }
type Induk = { id: string, rs_id: string, bulan: number, tahun: number }

export default function FormLaporanEdit({ masterPoli, induk, initialDetails }: { masterPoli: MasterPoli[], induk: Induk, initialDetails: LaporanDetail[] }) {
  const router = useRouter()
  
  const [step, setStep] = useState(2) // Start at step 2 for edit is often easier
  const [bulan] = useState(induk.bulan)
  const [tahun] = useState(induk.tahun)
  
  const [selectedPolis, setSelectedPolis] = useState<Set<string>>(() => {
    const pSet = new Set<string>()
    initialDetails.forEach(d => pSet.add(d.poli_id))
    return pSet
  })

  const [details, setDetails] = useState<Record<string, LaporanDetail>>(() => {
    const dMap: Record<string, LaporanDetail> = {}
    initialDetails.forEach(d => {
      dMap[d.poli_id] = {
        poli_id: d.poli_id,
        dalam_kota_laki: d.dalam_kota_laki.toString(),
        dalam_kota_perempuan: d.dalam_kota_perempuan.toString(),
        luar_kota_laki: d.luar_kota_laki.toString(),
        luar_kota_perempuan: d.luar_kota_perempuan.toString(),
        hari_buka: d.hari_buka.toString()
      }
    })
    return dMap
  })
  ...
  const handleNextToPoli = () => {
    if (!bulan || !tahun) { setErrorMsg("Bulan dan tahun wajib dipilih."); return }
    if (tahun < 2000 || tahun > 2100) { setErrorMsg("Tahun tidak valid."); return }
    setErrorMsg(null)
    setStep(2)
  }
  ...
  const togglePoli = (id: string) => {
    ...
    if (newSet.has(id)) {
      ...
      setDetails({
        ...details,
        [id]: { poli_id: id, dalam_kota_laki: '0', dalam_kota_perempuan: '0', luar_kota_laki: '0', luar_kota_perempuan: '0', hari_buka: '0' }
      })
    }
  ...
  const handleNextToForm = () => {
    if (selectedPolis.size === 0) { setErrorMsg("Pilih minimal satu poliklinik untuk dilaporkan."); return }
    setErrorMsg(null)
    setStep(3)
  }

  const maxDaysInMonth = useMemo(() => new Date(tahun, bulan, 0).getDate(), [bulan, tahun])

  const handleDetailChange = (poliId: string, field: keyof LaporanDetail, value: string) => {
    setDetails({ ...details, [poliId]: { ...details[poliId], [field]: value } })
  }
  ...
  const validateForm = (): boolean => {
    for (const [poliId, data] of Object.entries(details)) {
      ...
      for (const f of fields) {
        ...
        if (isNaN(val) || val < 0) {
          setErrorMsg(`${f.n} ${poliName} wajib diisi angka ≥ 0.`)
          return false
        }
      }
      const hb = parseInt(data.hari_buka)
      if (isNaN(hb) || hb < 0 || hb > maxDaysInMonth) {
        setErrorMsg(`Hari Buka ${poliName}: nilai ${data.hari_buka} melebihi jumlah hari bulan ${bulan}/${tahun} (${maxDaysInMonth} hari).`)
        return false
      }
    }
    return true
  ...
  const handleSubmit = async () => {
    ...
    setErrorMsg(null)
    try {
      const response = await fetch(`/api/laporan/${induk.id}`, {
        method: 'PUT',
      ...
      if (!response.ok) throw new Error(resData.error || "Gagal mengupdate laporan.")
      
      setSuccess(true)
      setTimeout(() => router.push('/rs/riwayat'), 2000)
    } catch (err: unknown) {
  ...
  if (success) {
    return (
      <div className="bg-white rounded-xl p-12 text-center shadow-sm border border-zinc-200 animate-in zoom-in duration-500">
        ...
        <h2 className="text-2xl font-bold text-zinc-900 mb-2">Laporan Berhasil Diperbarui!</h2>
            ...
            <div className="space-y-4">
              <div>
                ...
                <div className="relative">
                  <select 
                    value={bulan}
                    disabled
                    className="w-full pl-5 pr-12 py-3.5 rounded-2xl border-0 bg-zinc-100 shadow-inner outline-none font-medium text-zinc-500 opacity-80 cursor-not-allowed appearance-none"
                  >
              ...
              <div>
                ...
                <input 
                  ...
                  value={tahun}
                  disabled
                  className="w-full px-5 py-3.5 rounded-2xl border-0 bg-zinc-100 shadow-inner outline-none font-medium text-zinc-500 opacity-80 cursor-not-allowed"
                />
              </div>
            </div>
        ...
        {step === 3 && (
          <div className="animate-in slide-in-from-right-4">
            <div className="bg-amber-50 text-amber-800 p-4 rounded-xl text-sm mb-6 flex items-start border border-amber-200">
              ...
              <p>Maksimal Hari Buka untuk bulan ini adalah <strong>{maxDaysInMonth} hari</strong>.</p>
            </div>
            ...
            <div className="flex justify-between pt-10 mt-6 border-t border-zinc-100">
              <button onClick={() => setStep(2)} className="text-zinc-500 hover:text-zinc-800 px-6 py-2.5 font-bold transition-all flex items-center bg-zinc-50 hover:bg-zinc-100 rounded-xl"><ChevronLeft className="w-4 h-4 mr-2" /> Kembali</button>
              <button onClick={handleReview} className="bg-cyan-600 hover:bg-cyan-700 text-white px-8 py-3 rounded-xl font-bold transition-all flex items-center shadow-[0_4px_12px_rgba(8,145,178,0.2)] hover:shadow-[0_6px_16px_rgba(8,145,178,0.3)]">Review Laporan <ChevronRight className="w-4 h-4 ml-2" /></button>
            </div>
          </div>
        )}
          ...
          <div className="max-w-2xl mx-auto animate-in slide-in-from-right-4">
            <div className="bg-zinc-50 border-0 shadow-inner p-8 rounded-3xl mb-10">
              ...
              <p className="mt-8 text-sm text-zinc-500 bg-white p-4 rounded-xl border border-zinc-100 shadow-sm">
                <strong className="text-zinc-800">Pernyataan:</strong> Dengan menekan tombol Simpan, Anda menyatakan bahwa pembaruan data kunjungan pasien telah diisi dengan benar sesuai dengan rekapitulasi rumah sakit.
              </p>
            </div>
            <div className="flex justify-between">
              <button onClick={() => setStep(3)} disabled={isSubmitting} className="text-zinc-500 hover:text-zinc-800 px-6 py-2.5 font-bold transition-all flex items-center disabled:opacity-50 bg-zinc-50 hover:bg-zinc-100 rounded-xl"><ChevronLeft className="w-4 h-4 mr-2" /> Edit Kembali</button>
              <button onClick={handleSubmit} disabled={isSubmitting} className="bg-cyan-600 hover:bg-cyan-700 text-white px-10 py-4 rounded-xl font-bold transition-all shadow-[0_4px_12px_rgba(8,145,178,0.2)] hover:shadow-[0_6px_16px_rgba(8,145,178,0.3)] flex items-center disabled:opacity-70 text-base">
                {isSubmitting ? <span className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin mr-2" /> : <Save className="w-5 h-5 mr-2" />} Simpan Perubahan
              </button>
            </div>
          </div>
        )}
